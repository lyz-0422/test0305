...
└── notes.html          ← 課程學習筆記（互動式 Demo）
    ├── JavaScript       JS·01 ~ JS·04
    └── CSS Layout       CSS·01 ~ CSS·04
```
...
| 類別 | 篇號 | 主題 | Demo 功能 |
|------|------|------|-----------|
| JavaScript | JS·01 | 變數、型別、函式 | 四則運算計算機 |
| JavaScript | JS·02 | 陣列、物件、JSON | Push / Map 陣列操作面板 |
| JavaScript | JS·03 | DOM 操作與事件 | 動態待辦事項列表 |
| JavaScript | JS·04 | 計時器與動態效果 | 碼錶（Start / Pause / Reset） |
| CSS Layout | CSS·01 | FlexBox | `justify-content` 即時調整預覽 |
| CSS Layout | CSS·02 | Grid | `grid-template-columns` 即時切換 |
| CSS Layout | CSS·03 | RWD 響應式設計 | 手機 / 電腦版面切換模擬 |
| CSS Layout | CSS·04 | CSS Animation | Spin / Bounce / Pulse 動畫選擇器 |
...
| **JavaScript ES6+** | `IntersectionObserver` · `requestAnimationFrame` · `URLSearchParams` · Canvas 2D |
